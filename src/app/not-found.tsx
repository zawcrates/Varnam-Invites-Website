import React from 'react';
import Link from 'next/link';
import Navbar from '@/components/Navbar';
import Footer from '@/components/Footer';
import { ShoppingBag, ArrowLeft } from 'lucide-react';

export default function NotFound() {
  return (
    <div className="flex flex-col min-h-screen bg-background">
      <Navbar />

      <main className="flex-grow flex items-center justify-center p-6 pt-32">
        <div className="text-center bg-white border border-gold-medium/15 p-12 rounded-3xl max-w-md w-full luxury-shadow text-left">
          <div className="w-16 h-16 bg-gold-light/20 rounded-full flex items-center justify-center mx-auto mb-6 text-gold-dark">
            <ShoppingBag className="w-8 h-8" />
          </div>

          <h1 className="font-sansflex text-3xl font-bold text-luxury-dark mb-2 text-center">404 - Page Not Found</h1>
          <p className="text-xs text-foreground/60 mb-8 leading-relaxed text-center">
            This digital path does not exist, or the invitation page you are trying to view has been moved or is currently unpublished.
          </p>

          <div className="flex flex-col gap-3">
            <Link
              href="/"
              className="inline-flex justify-center items-center gap-2 text-xs uppercase tracking-widest font-bold bg-luxury-dark hover:bg-gold-dark text-gold-light hover:text-white py-4 rounded-full transition-all duration-300"
            >
              <ArrowLeft className="w-4 h-4" />
              <span>Back to home</span>
            </Link>
            <Link
              href="/templates"
              className="inline-flex justify-center items-center text-xs uppercase tracking-widest font-bold border border-gold-medium/20 hover:border-gold-medium/50 text-luxury-dark py-4 rounded-full transition-all duration-300 text-center"
            >
              Browse templates
            </Link>
          </div>
        </div>
      </main>

      <Footer />
    </div>
  );
}
